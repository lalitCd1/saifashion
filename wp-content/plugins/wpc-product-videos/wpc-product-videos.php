...
! defined( 'WPCPV_VERSION' ) && define( 'WPCPV_VERSION', '1.0.5' );
			...
			class WPCleverWpcpv {
				...
				function __construct() {
					...
					add_filter( 'woocommerce_single_product_image_thumbnail_html', [
						$this,
						'image_thumbnail_html'
					], 10, 2 );
				}

				function enqueue_scripts() {
					// light gallery
					wp_enqueue_script( 'lightgallery', WPCPV_URI . 'assets/libs/lightgallery/js/lightgallery-all.min.js', array( 'jquery' ), WPCPV_VERSION, true );
					...
					wp_enqueue_script( 'wpcpv-frontend', WPCPV_URI . 'assets/js/frontend.js', array( 'jquery' ), WPCPV_VERSION, true );
				}

				function admin_menu() {
					add_submenu_page( 'wpclever', esc_html__( 'WPC Product Videos', 'wpc-product-videos' ), esc_html__( 'Product Videos', 'wpc-product-videos' ), 'manage_options', 'wpclever-wpcpv', array(
						$this,
						'admin_menu_content'
					) );
				}
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_desc about-text">
                            <p>
                                ...
                                <a href="<?php echo esc_url( WPCPV_REVIEWS ); ?>"
                                   target="_blank"><?php esc_html_e( 'Reviews', 'wpc-product-videos' ); ?></a> | <a
                                        href="<?php echo esc_url( WPCPV_CHANGELOG ); ?>"
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                <a href="<?php echo admin_url( 'admin.php?page=wpclever-wpcpv&tab=how' ); ?>"
                        ...
                        <div class="wpclever_settings_page_content">
							<?php if ( $active_tab === 'how' ) { ?>
                                <div class="wpclever_settings_page_content_text">
                                    ...
                                    <p><img src="<?php echo WPCPV_URI; ?>assets/images/how-01.jpg"/></p>
                                    <p>
										<?php esc_html_e( 'Then the video will be linked to the image on the product page.', 'wpc-product-videos' ); ?>
                                    </p>
                                    <p><img src="<?php echo WPCPV_URI; ?>assets/images/how-02.jpg"/></p>
                                </div>
							<?php } ?>
                        </div>
                    </div>
				...
				function attachment_field_video( $form_fields, $post ) {
					$form_fields['wpcpv-video-url'] = array(
				...
				function image_thumbnail_html( $html, $attachment_id ) {
					...
					if ( $video = get_post_meta( $attachment_id, 'wpcpv_video_url', true ) ) {
						$html = str_replace( '</div>', '<span class="wpcpv-item wpcpv-item-video" data-src="' . esc_url( $video ) . '"><img src="' . esc_url( $thumbnail_src[0] ) . '"/></span></div>', $html );
					} else {
						$html = str_replace( '</div>', '<span class="wpcpv-item wpcpv-item-image" data-src="' . wp_get_attachment_url( $attachment_id ) . '"><img src="' . esc_url( $thumbnail_src[0] ) . '"/></span></div>', $html );
					}
